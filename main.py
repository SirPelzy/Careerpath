...
import uuid
import datetime
from datetime import datetime, timedelta # Ensure both are imported correctly
...
import requests # For Brevo API & Paystack API
import random
import string
import sentry_sdk
...
# --- Configuration ---
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'fallback_secret_key_needs_to_be_set_properly')
...
app.config['MAIL_USERNAME'] = os.environ.get('MAIL_USERNAME') # Brevo login email needed for config checks below
...
db.init_app(app)
migrate = Migrate(app, db) # Initialize Flask-Migrate AFTER db init
...
def dashboard():
    ...
    if target_path:
        ...
        if current_path_step_ids:
            ...
            for milestone in milestones: # Use already loaded milestones
                # Use count() for total, no need to query IDs again if using intersection below
                total_steps_in_milestone = Step.query.filter_by(milestone_id=milestone.id).count()
                if total_steps_in_milestone > 0:
                    # Get IDs of steps already loaded via eager loading
                    milestone_step_ids = {step.id for step in milestone.steps} # Efficient now
                    ...
                    milestone_progress[milestone.id] = {
                        'completed': completed_in_milestone, 'total': total_steps_in_milestone, 'percent': percent_complete
                    }
                else:
            ...
            style_to_type_map = {
                'Visual': ['Video', 'Project', 'Course', 'Guide', 'Platform'], 'Auditory': ['Video', 'Course'],
...
def register():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            try:
                ...
                expiry = datetime.utcnow() + timedelta(minutes=15) # Corrected usage
    ...
    return render_template('register.html', title='Register', form=form, is_homepage=False) # Dark navbar
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and user.check_password(form.password.data):
            ...
            user.last_login = datetime.utcnow() # Corrected usage
            try: db.session.commit()
            except Exception as e: db.session.rollback(); print(f"Error updating last_login: {e}")

            # --- Check if email verified AFTER login ---
    ...
    return render_template('login.html', title='Login', form=form, is_homepage=False) # Dark navbar
    ...
    if form.validate_on_submit():
        try:
            ...
            if form.cv_upload.data:
                ...
                base_filename = secure_filename(file.filename); unique_id = uuid.uuid4().hex
            ...
            return redirect(url_for('dashboard'))
             ...
             db.session.rollback(); print(f"Error during onboarding form save: {e}")
...
def recommendation_test():
    ...
    if form.validate_on_submit():
        ...
        elif answers['q1'] == 'B': scores["UX/UI Design"] += 1; elif answers['q1'] == 'C': scores["Software Engineering"] += 1; elif answers['q1'] == 'D': scores["Cybersecurity"] += 1
        if answers['q2'] == 'A': scores["Data Analysis / Analytics"] += 1
        elif answers['q2'] == 'B': scores["UX/UI Design"] += 1; elif answers['q2'] == 'C': scores["Software Engineering"] += 1; elif answers['q2'] == 'D': scores["Cybersecurity"] += 1
        if answers['q3'] == 'A': scores["Data Analysis / Analytics"] += 1
        elif answers['q3'] == 'B': scores["UX/UI Design"] += 1; elif answers['q3'] == 'C': scores["Software Engineering"] += 1; elif answers['q3'] == 'D': scores["Cybersecurity"] += 1
        if answers['q4'] == 'A': scores["Data Analysis / Analytics"] += 1
        elif answers['q4'] == 'B': scores["UX/UI Design"] += 1; elif answers['q4'] == 'C': scores["Software Engineering"] += 1; elif answers['q4'] == 'D': scores["Cybersecurity"] += 1
        ...
        session['recommendation_scores'] = scores
...
def recommendation_results():
    ...
    scores = session.pop('recommendation_scores', None)
...
def verify_code_entry():
    ...
    if form.validate_on_submit():
        if not email: flash("Could not identify user. Please try logging in.", "warning"); return redirect(url_for('login'))
        user = User.query.filter_by(email=email).first()
        submitted_code = form.code.data
        if not user: flash("User not found. Please register or check the email address.", "danger"); return redirect(url_for('login'))

        if user.verification_code == submitted_code and user.verification_code_expiry and user.verification_code_expiry > datetime.utcnow(): # Corrected datetime usage
            try:
                ...
                return redirect(url_for('login'))
                 ...
                 db.session.rollback(); print(f"Error verifying email for {email}: {e}")
...
def verify_code_required():
    ...
    if form.validate_on_submit():
        submitted_code = form.code.data
        if current_user.verification_code == submitted_code and current_user.verification_code_expiry and current_user.verification_code_expiry > datetime.utcnow(): # Corrected datetime usage
            try:
                ...
                return redirect(url_for('dashboard')) # Redirect to dashboard on success
                 ...
                 db.session.rollback(); print(f"Error verifying email post-login for {current_user.email}: {e}")
                 flash("An error occurred during verification. Please try again.", 'danger')
        else:
            flash("Invalid or expired verification code. A new code may have been sent if you attempted login again.", "danger")
            return redirect(url_for('verify_code_required')) # Redirect back to GET
    ...
    if form.validate_on_submit():
        ...
        if user:
            try:
                ...
                if email_sent: flash('An email has been sent with instructions to reset your password.', 'info')
                else: flash('Could not send password reset email. Please try again later or contact support.', 'danger')
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        try:
            ...
            db.session.rollback(); print(f"Error resetting password for user {user.id}: {e}")
    ...
    try:
        if user_status:
            if user_status.status == 'completed':
                user_status.status = 'not_started'; user_status.completed_at = None; new_status = 'not_started'
                flash_message = f'Step "{step.name}" marked as not started.'
            else:
                user_status.status = 'completed'; user_status.completed_at = datetime.utcnow() # Corrected
                new_status = 'completed'; flash_message = f'Step "{step.name}" marked as completed!'
        else:
            user_status = UserStepStatus(user_id=current_user.id, step_id=step.id, status='completed', completed_at=datetime.utcnow()) # Corrected
        ...
        if step.milestone:
            milestone = step.milestone; m_id = milestone.id
            all_m_step_ids_q = Step.query.filter_by(milestone_id=m_id).with_entities(Step.id)
            all_m_step_ids = {sid for sid, in all_m_step_ids_q.all()}; m_total = len(all_m_step_ids)
            if m_total > 0:
        ...
        return jsonify({
            'success': True, 'new_status': new_status, 'step_id': step.id, 'milestone_id': step.milestone_id,
            'message': flash_message, 'milestone_completed': milestone_completed_now,
            'milestone_progress': updated_milestone_progress, 'overall_progress': updated_overall_progress })
    except Exception as e:
        db.session.rollback(); print(f"Error updating step status via AJAX for user {current_user.id}, step {step_id}: {e}")
...
def add_portfolio_item():
    ...
    if request.method == 'GET':
        ...
        if step_id_from_url:
            ...
            else: step_id_from_url = None; flash("Invalid associated step ID provided.", "warning")
        # Add similar for milestone_id if linking from milestones later
    if form.validate_on_submit():
        ...
        try: db.session.add(new_item); db.session.commit(); flash('Portfolio item added successfully!', 'success'); return redirect(url_for('portfolio'))
        except Exception as e: db.session.rollback(); print(f"Error adding portfolio item to DB: {e}"); flash('Error saving portfolio item.', 'danger')
...
def edit_portfolio_item(item_id):
    ...
    if form.validate_on_submit():
        ...
        try:
            db.session.commit(); flash('Portfolio item updated successfully!', 'success')
        ...
        except Exception as e: db.session.rollback(); print(f"Error updating portfolio item {item_id}: {e}"); flash('Error updating portfolio item.', 'danger')
...
def delete_portfolio_item(item_id):
    ...
    except Exception as e: db.session.rollback(); print(f"Error deleting portfolio item {item_id}: {e}"); flash('Error deleting portfolio item.', 'danger')
    return redirect(url_for('portfolio'))
...
def unlink_portfolio_item(item_id):
    ...
    if item.associated_step_id is None and item.associated_milestone_id is None: flash("This item is not linked.", "info")
        ...
        try: item.associated_step_id = None; item.associated_milestone_id = None; db.session.commit(); flash("Portfolio item unlinked.", "success")
...
def download_portfolio_file(item_id):
    ...
    except FileNotFoundError: flash("Error: File not found on server.", "danger"); return redirect(url_for('portfolio')) # Added flash
    except Exception as e: print(f"Error sending portfolio file {filename}: {e}"); flash("Error downloading file.", "danger"); return redirect(url_for('portfolio')) # Added flash
...
            return redirect(url_for('profile'))
        except Exception as e:
             db.session.rollback(); print(f"Error during profile update: {e}")
