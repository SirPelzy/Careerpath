...
# --- <<< TEMPORARY ADMIN ROUTE TO DROP, CREATE, & SEED >>> ---
# !! DANGER !! DELETES ALL DATA. REMOVE AFTER USE. !! DANGER !!
# !! REPLACE ALL #placeholder_... URLs BEFORE RUNNING !!
INIT_DB_SECRET_KEY = os.environ.get('INIT_DB_SECRET_KEY', 'replace-this-with-a-very-STRONG-secret-key-12345')

@app.route(f'/admin/init-db-reset/{INIT_DB_SECRET_KEY}') # Changed path slightly for safety
def init_database_reset():
    """Temporary route to DROP ALL TABLES, recreate them, and seed path data."""
    print("!!! DANGER: Attempting FULL DATABASE RESET AND SEED !!!")
    try:
        with app.app_context():
            ...
            db.create_all() # This will include step_type, email_verified, plan etc.
            print("Tables created.")
            # --- End Create ---

            # --- Seed Initial Career Paths ---
            print("Populating initial Career Paths...")
            # Check if they already exist (shouldn't after drop_all, but safe practice)
            if not CareerPath.query.first():
                ...
                print("Career Paths already exist (should not happen after drop_all).")
            # --- End Career Path Seed ---

            # --- Seed Data Analytics Path ---
            print("Attempting to seed Data Analysis path...")
            da_path = CareerPath.query.filter_by(name="Data Analysis / Analytics").first()
            if da_path:
                resources_to_add_da = []
                try:
                    # M1: Introduction & Foundation
                    ...
                    resources_to_add_da.append(Resource(name="Intro Course (Udemy)", url="#placeholder_da_m1_s1_udemy", resource_type="Course", step_id=s1_1_da.id))
                    resources_to_add_da.append(Resource(name="Determining Right Tech Career (Medium)", url="#placeholder_da_m1_s2_medium", resource_type="Article", step_id=s1_2_da.id))
                    resources_to_add_da.append(Resource(name="Best Practices for Tech Learning (Medium)", url="#placeholder_da_m1_s3_medium", resource_type="Article", step_id=s1_3_da.id))
                    ...
                    resources_to_add_da.append(Resource(name="Excel Basic Tutorial (Alex YT - Placeholder)", url="#placeholder_da_m2_s1_alexyt", resource_type="Video", step_id=s2_1_da.id))
                    resources_to_add_da.append(Resource(name="Excel Lookup Formulas (LeilaG YT - Placeholder)", url="#placeholder_da_m2_s2_leilayt", resource_type="Video", step_id=s2_2_da.id))
                    resources_to_add_da.append(Resource(name="Excel Charts (LeilaG YT - Placeholder)", url="#placeholder_da_m2_s3_leilayt", resource_type="Video", step_id=s2_3_da.id))
                    resources_to_add_da.append(Resource(name="Data Prep Course (DataCamp)", url="#placeholder_da_m2_s4_datacamp", resource_type="Course", step_id=s2_4_da.id))
                    resources_to_add_da.append(Resource(name="Excel Guided Project (Coursera - Placeholder)", url="#placeholder_da_m2_s5_coursera", resource_type="Project", step_id=s2_5_da.id))
                    ...
                    resources_to_add_da.append(Resource(name="SQL Basic Tutorial (Alex YT - Placeholder)", url="#placeholder_da_m3_s1_alexyt", resource_type="Video", step_id=s3_1_da.id))
                    resources_to_add_da.append(Resource(name="SQL Intermediate Tutorial (Alex YT - Placeholder)", url="#placeholder_da_m3_s2_alexyt", resource_type="Video", step_id=s3_2_da.id))
                    resources_to_add_da.append(Resource(name="Advanced SQL Tutorial (Alex YT - Placeholder)", url="#placeholder_da_m3_s3_alexyt", resource_type="Video", step_id=s3_3_da.id))
                    resources_to_add_da.append(Resource(name="SQL Tutorial", url="#placeholder_da_m3_s4_datalemur", resource_type="Tutorial", step_id=s3_4_da.id))
                    resources_to_add_da.append(Resource(name="SQL Practice Questions", url="#placeholder_da_m3_s5_datalemur", resource_type="Practice", step_id=s3_5_da.id))
                    ...
                    resources_to_add_da.append(Resource(name="Tableau Free Training Videos", url="#placeholder_da_m4_s1_tableau", resource_type="Course", step_id=s4_1_da.id))
                    resources_to_add_da.append(Resource(name="Tableau for Beginners (Alex YT - Placeholder)", url="#placeholder_da_m4_s2_alexyt", resource_type="Video", step_id=s4_2_da.id))
                    resources_to_add_da.append(Resource(name="Creating Dashboards (Alex YT - Placeholder)", url="#placeholder_da_m4_s3_alexyt", resource_type="Project", step_id=s4_3_da.id))
                    resources_to_add_da.append(Resource(name="Tableau Beginner Project (Alex YT - Placeholder)", url="#placeholder_da_m4_s4_alexyt", resource_type="Project", step_id=s4_4_da.id))

                    # M5: Data Visualization with Power BI
                    ...
                    resources_to_add_da.append(Resource(name="Microsoft Learn Power BI Modules (MS - Placeholder)", url="#placeholder_da_m5_s1_mslearn", resource_type="Course", step_id=s5_1_da.id))

                    # M6: Programming with Python for Data Analysis
                    ...
                    resources_to_add_da.append(Resource(name="Python for Beginners (Alex YT - Placeholder)", url="#placeholder_da_m6_s1_alexyt", resource_type="Video", step_id=s6_1_da.id))
                    ...
                    resources_to_add_da.append(Resource(name="Python Project (Alex YT - Placeholder)", url="#placeholder_da_m6_s4_alexyt", resource_type="Project", step_id=s6_4_da.id))
                    resources_to_add_da.append(Resource(name="Python Web Scraping Basics (Alex YT - Placeholder)", url="#placeholder_da_m6_s5_alexyt", resource_type="Video", step_id=s6_5_da.id))

                    # M7: Building Your Data Analytics Portfolio
                    ...
                    resources_to_add_da.append(Resource(name="How I Structure My Data Analytics Reports (Medium - Placeholder)", url="#placeholder_da_m7_s1_medium", resource_type="Article", step_id=s7_1_da.id))
                    resources_to_add_da.append(Resource(name="How to use Kaggle (YT - Placeholder)", url="#placeholder_da_m7_s2_kaggle_yt", resource_type="Guide", step_id=s7_2_da.id))
                    resources_to_add_da.append(Resource(name="How to use GitHub (YT - Placeholder)", url="#placeholder_da_m7_s2_github_yt", resource_type="Guide", step_id=s7_2_da.id))
                    resources_to_add_da.append(Resource(name="How to use Google Sites (YT - Placeholder)", url="#placeholder_da_m7_s2_gsites_yt", resource_type="Guide", step_id=s7_2_da.id))
                    ...
                    # M8: Gaining Practical Experience
                    ...
                    s8_2_da = Step(name="Complete 1-2 Relevant Virtual Internships", sequence=20, estimated_time_minutes=600, milestone_id=m8_da.id, step_type='Internship') # Changed type
                    db.session.add_all([s8_1_da, s8_2_da]); db.session.flush()
                    resources_to_add_da.append(Resource(name="Forage Virtual Experiences (Forage - Placeholder)", url="#placeholder_da_m8_s1_forage", resource_type="Platform", step_id=s8_1_da.id))

                    # M9: Resume & Job Application Strategy
                    ...
                    resources_to_add_da.append(Resource(name="How to create DA resume (YT - Placeholder)", url="#placeholder_da_m9_s1_resume_yt", resource_type="Guide", step_id=s9_1_da.id))
                    resources_to_add_da.append(Resource(name="Free Resume Template (Placeholder)", url="#placeholder_da_m9_s1_template", resource_type="Resource", step_id=s9_1_da.id))
                    resources_to_add_da.append(Resource(name="LinkedIn Job Tips 1 (YT - Placeholder)", url="#placeholder_da_m9_s2_li_yt1", resource_type="Guide", step_id=s9_2_da.id))
                    resources_to_add_da.append(Resource(name="LinkedIn Job Tips 2 (YT - Placeholder)", url="#placeholder_da_m9_s2_li_yt2", resource_type="Guide", step_id=s9_2_da.id))
                    resources_to_add_da.append(Resource(name="LinkedIn Optimization Thread (Twitter - Placeholder)", url="#placeholder_da_m9_s2_li_twitter", resource_type="Guide", step_id=s9_2_da.id))

                    # M10: Interview Preparation
                    ...
                    resources_to_add_da.append(Resource(name="Interview Red Flags Thread (Twitter - Placeholder)", url="#placeholder_da_m10_s1_flags_twitter", resource_type="Guide", step_id=s10_1_da.id))
                    resources_to_add_da.append(Resource(name="DA Interview Questions Thread (Twitter - Placeholder)", url="#placeholder_da_m10_s2_qs_twitter", resource_type="Guide", step_id=s10_2_da.id))

                    if resources_to_add_da: db.session.add_all(resources_to_add_da)
                    db.session.commit()
                    print(f"Path '{da_path.name}' seeded successfully.")
                except Exception as e:
                    db.session.rollback(); print(f"Error seeding path '{da_path.name}': {e}")
            else: print("DA Path not found.")
            # --- END DA Seed ---


            # --- Re-Seed UX/UI Design Path ---
            print("Attempting to seed UX/UI Design path...")
            uxui_path = CareerPath.query.filter_by(name="UX/UI Design").first()
            if uxui_path:
                resources_to_add_ux = []
                try:
                    # M1: Introduction to UX/UI
                    ...
                    resources_to_add_ux.append(Resource(name="UX vs UI Explained (CareerFoundry - Placeholder)", url="#placeholder_uxui_m1_s1_cf", resource_type="Article", step_id=s1_1_ux.id))
                    resources_to_add_ux.append(Resource(name="Google UX Design Certificate Intro (Coursera - Placeholder)", url="#placeholder_uxui_m1_s2_google", resource_type="Course", step_id=s1_2_ux.id))
                    resources_to_add_ux.append(Resource(name="Design Thinking Overview (IDEO - Placeholder)", url="#placeholder_uxui_m1_s3_ideo", resource_type="Resource", step_id=s1_3_ux.id))

                    # M2: Core Design Principles
                    ...
                    resources_to_add_ux.append(Resource(name="Laws of UX (Website - Placeholder)", url="#placeholder_uxui_m2_s1_lawsux", resource_type="Resource", step_id=s2_1_ux.id))
                    resources_to_add_ux.append(Resource(name="Color Theory (Interaction Design Foundation - Placeholder)", url="#placeholder_uxui_m2_s2_idf", resource_type="Article", step_id=s2_2_ux.id))
                    resources_to_add_ux.append(Resource(name="Typography Guide (Material Design - Placeholder)", url="#placeholder_uxui_m2_s3_material", resource_type="Guide", step_id=s2_3_ux.id))
                    resources_to_add_ux.append(Resource(name="10 Usability Heuristics (NN/g - Placeholder)", url="#placeholder_uxui_m2_s4_nng", resource_type="Article", step_id=s2_4_ux.id))

                    # M3: User Research Fundamentals
                    ...
                    resources_to_add_ux.append(Resource(name="Personas Guide (Interaction Design Foundation - Placeholder)", url="#placeholder_uxui_m3_s1_idf", resource_type="Article", step_id=s3_1_ux.id))
                    resources_to_add_ux.append(Resource(name="User Interview Guide (NN/g - Placeholder)", url="#placeholder_uxui_m3_s2_nng", resource_type="Article", step_id=s3_2_ux.id))
                    resources_to_add_ux.append(Resource(name="Survey Guide (SurveyMonkey - Placeholder)", url="#placeholder_uxui_m3_s3_sm", resource_type="Guide", step_id=s3_3_ux.id))
                    resources_to_add_ux.append(Resource(name="Usability Testing 101 (NN/g - Placeholder)", url="#placeholder_uxui_m3_s4_nng", resource_type="Article", step_id=s3_4_ux.id))
                    ...
                    resources_to_add_ux.append(Resource(name="Complete Beginner's Guide to IA (UX Booth - Placeholder)", url="#placeholder_uxui_m4_s1_uxbooth", resource_type="Article", step_id=s4_1_ux.id))
                    resources_to_add_ux.append(Resource(name="Sitemaps Tutorial (Figma - Placeholder)", url="#placeholder_uxui_m4_s2_figma", resource_type="Tutorial", step_id=s4_2_ux.id))
                    resources_to_add_ux.append(Resource(name="User Flow Guide (Adobe XD - Placeholder)", url="#placeholder_uxui_m4_s3_adobe", resource_type="Guide", step_id=s4_3_ux.id))
                    resources_to_add_ux.append(Resource(name="Card Sorting Intro (NN/g - Placeholder)", url="#placeholder_uxui_m4_s4_nng", resource_type="Article", step_id=s4_4_ux.id))

                    # M5: Wireframing & Prototyping
                    ...
                    resources_to_add_ux.append(Resource(name="Wireframing Guide (CareerFoundry - Placeholder)", url="#placeholder_uxui_m5_s1_cf", resource_type="Guide", step_id=s5_1_ux.id))
                    resources_to_add_ux.append(Resource(name="Low-Fi Wireframing Tools (Blog - Placeholder)", url="#placeholder_uxui_m5_s2_blog", resource_type="Article", step_id=s5_2_ux.id))
                    resources_to_add_ux.append(Resource(name="Hi-Fi Wireframing Tutorial (Figma YT - Placeholder)", url="#placeholder_uxui_m5_s3_figmayt", resource_type="Video", step_id=s5_3_ux.id))
                    resources_to_add_ux.append(Resource(name="Prototyping in Figma (Figma Docs - Placeholder)", url="#placeholder_uxui_m5_s4_figmadocs", resource_type="Documentation", step_id=s5_4_ux.id))

                    # M6: Mastering Design Tools (Figma)
                    ...
                    resources_to_add_ux.append(Resource(name="Figma Beginners Tutorial (YT - Placeholder)", url="#placeholder_uxui_m6_s1_yt", resource_type="Video", step_id=s6_1_ux.id))
                    resources_to_add_ux.append(Resource(name="Figma Auto Layout Guide (Figma - Placeholder)", url="#placeholder_uxui_m6_s2_figma", resource_type="Guide", step_id=s6_2_ux.id))
                    resources_to_add_ux.append(Resource(name="Figma Components Tutorial (YT - Placeholder)", url="#placeholder_uxui_m6_s3_yt", resource_type="Video", step_id=s6_3_ux.id))
                    resources_to_add_ux.append(Resource(name="Top Figma Plugins (Blog - Placeholder)", url="#placeholder_uxui_m6_s4_blog", resource_type="Article", step_id=s6_4_ux.id))
                    resources_to_add_ux.append(Resource(name="Design a Mobile App (YT Project - Placeholder)", url="#placeholder_uxui_m6_s5_yt", resource_type="Project", step_id=s6_5_ux.id))

                    # M7: Visual Design & UI Details
                    ...
                    resources_to_add_ux.append(Resource(name="Style Guide Examples (Blog - Placeholder)", url="#placeholder_uxui_m7_s1_blog", resource_type="Article", step_id=s7_1_ux.id))
                    resources_to_add_ux.append(Resource(name="Design Systems Intro (InVision - Placeholder)", url="#placeholder_uxui_m7_s2_invision", resource_type="Article", step_id=s7_2_ux.id))
                    resources_to_add_ux.append(Resource(name="WCAG Overview (W3C - Placeholder)", url="#placeholder_uxui_m7_s3_w3c", resource_type="Documentation", step_id=s7_3_ux.id))
                    resources_to_add_ux.append(Resource(name="Free UI Kits for Figma (Resource - Placeholder)", url="#placeholder_uxui_m7_s4_kits", resource_type="Resource", step_id=s7_4_ux.id))

                    # M8: Building Your UX/UI Portfolio
                    ...
                    resources_to_add_ux.append(Resource(name="How to Choose Projects (Blog - Placeholder)", url="#placeholder_uxui_m8_s1_blog", resource_type="Article", step_id=s8_1_ux.id))
                    resources_to_add_ux.append(Resource(name="UX Case Study Guide (Medium - Placeholder)", url="#placeholder_uxui_m8_s2_medium", resource_type="Article", step_id=s8_2_ux.id))
                    ...
                    resources_to_add_ux.append(Resource(name="Designer/Developer Collaboration (Abstract - Placeholder)", url="#placeholder_uxui_m9_s1_abstract", resource_type="Article", step_id=s9_1_ux.id))
                    resources_to_add_ux.append(Resource(name="Design Specs Guide (Zeplin Blog - Placeholder)", url="#placeholder_uxui_m9_s2_zeplin", resource_type="Article", step_id=s9_2_ux.id))
                    resources_to_add_ux.append(Resource(name="Figma Inspect Mode (Figma Docs - Placeholder)", url="#placeholder_uxui_m9_s3_figma", resource_type="Documentation", step_id=s9_3_ux.id))

                    # M10: Job Search & Interview Prep (UX/UI)
                    ...
                    resources_to_add_ux.append(Resource(name="UX Resume Tips (NN/g - Placeholder)", url="#placeholder_uxui_m10_s1_nng", resource_type="Article", step_id=s10_1_ux.id))
                    resources_to_add_ux.append(Resource(name="Portfolio Review Prep (Medium - Placeholder)", url="#placeholder_uxui_m10_s2_medium", resource_type="Article", step_id=s10_2_ux.id))
                    resources_to_add_ux.append(Resource(name="UX Interview Questions (Toptal - Placeholder)", url="#placeholder_uxui_m10_s3_toptal", resource_type="Article", step_id=s10_3_ux.id))
                    resources_to_add_ux.append(Resource(name="Whiteboard Challenge Guide (YT - Placeholder)", url="#placeholder_uxui_m10_s4_yt", resource_type="Video", step_id=s10_4_ux.id))

                    if resources_to_add_ux: db.session.add_all(resources_to_add_ux)
                    db.session.commit()
                    print(f"Path '{uxui_path.name}' seeded successfully.")
                except Exception as e:
                    db.session.rollback(); print(f"Error seeding path '{uxui_path.name}': {e}")
            else: print("UXUI Path not found.")
            # --- <<< END UX/UI SEEDING BLOCK >>> ---


            # --- Re-Seed Cybersecurity Path ---
            print("Attempting to seed Cybersecurity path...")
            ...
            if cyber_path:
                ...
                try:
                    ...
                    resources_to_add_cyber.append(Resource(name="CompTIA IT Fundamentals (Exam Objectives)", url="#placeholder_cyber_m1_s1_itf", resource_type="Guide", step_id=s1_1_cy.id))
                    resources_to_add_cyber.append(Resource(name="OSI Model Explained (Video)", url="#placeholder_cyber_m1_s2_osi", resource_type="Video", step_id=s1_2_cy.id))
                    resources_to_add_cyber.append(Resource(name="IP Addressing Tutorial (Website)", url="#placeholder_cyber_m1_s3_ip", resource_type="Tutorial", step_id=s1_3_cy.id))
                    resources_to_add_cyber.append(Resource(name="Network Protocols Overview (Article)", url="#placeholder_cyber_m1_s4_protocols", resource_type="Article", step_id=s1_4_cy.id))

                    # M2: Intro to Cybersecurity Principles
                    ...
                    resources_to_add_cyber.append(Resource(name="CIA Triad Explained (Article)", url="#placeholder_cyber_m2_s1_cia", resource_type="Article", step_id=s2_1_cy.id))
                    resources_to_add_cyber.append(Resource(name="Common Cyber Threats (Cybrary - Placeholder)", url="#placeholder_cyber_m2_s2_cybrary", resource_type="Resource", step_id=s2_2_cy.id))
                    resources_to_add_cyber.append(Resource(name="Cryptography Basics (Khan Academy - Placeholder)", url="#placeholder_cyber_m2_s3_khan", resource_type="Course", step_id=s2_3_cy.id))
                    resources_to_add_cyber.append(Resource(name="Thinking Like an Attacker (Blog)", url="#placeholder_cyber_m2_s4_blog", resource_type="Article", step_id=s2_4_cy.id))

                    # M3: Operating Systems Security
                    ...
                    resources_to_add_cyber.append(Resource(name="Linux Journey (Website)", url="#placeholder_cyber_m3_s1_linuxjourney", resource_type="Tutorial", step_id=s3_1_cy.id))
                    resources_to_add_cyber.append(Resource(name="Linux Permissions Explained (Article)", url="#placeholder_cyber_m3_s2_linuxperm", resource_type="Article", step_id=s3_2_cy.id))
                    resources_to_add_cyber.append(Resource(name="Windows Security Settings (MS Docs)", url="#placeholder_cyber_m3_s3_msdocs", resource_type="Documentation", step_id=s3_3_cy.id))
                    resources_to_add_cyber.append(Resource(name="CIS Benchmarks Intro", url="#placeholder_cyber_m3_s4_cis", resource_type="Resource", step_id=s3_4_cy.id))

                    # M4: Network Security Fundamentals
                    ...
                    resources_to_add_cyber.append(Resource(name="Firewalls Explained (Video)", url="#placeholder_cyber_m4_s1_yt", resource_type="Video", step_id=s4_1_cy.id))
                    resources_to_add_cyber.append(Resource(name="How VPNs Work (Article)", url="#placeholder_cyber_m4_s2_vpn", resource_type="Article", step_id=s4_2_cy.id))
                    resources_to_add_cyber.append(Resource(name="IDS vs IPS (Article)", url="#placeholder_cyber_m4_s3_idsips", resource_type="Article", step_id=s4_3_cy.id))
                    resources_to_add_cyber.append(Resource(name="Wi-Fi Security Guide", url="#placeholder_cyber_m4_s4_wifi", resource_type="Guide", step_id=s4_4_cy.id))

                    # M5: Security Tools & Technologies Intro
                    ...
                    resources_to_add_cyber.append(Resource(name="Nmap Basics (TryHackMe - Placeholder)", url="#placeholder_cyber_m5_s1_thm", resource_type="Practice", step_id=s5_1_cy.id))
                    resources_to_add_cyber.append(Resource(name="Wireshark Tutorial (Video)", url="#placeholder_cyber_m5_s2_yt", resource_type="Video", step_id=s5_2_cy.id))
                    resources_to_add_cyber.append(Resource(name="Vulnerability Scanning Overview (Article)", url="#placeholder_cyber_m5_s3_vulnscan", resource_type="Article", step_id=s5_3_cy.id))
                    resources_to_add_cyber.append(Resource(name="What is SIEM? (Splunk)", url="#placeholder_cyber_m5_s4_splunk", resource_type="Article", step_id=s5_4_cy.id))

                    # M6: Threats, Vulnerabilities & Risk Management
                    ...
                    resources_to_add_cyber.append(Resource(name="Malware Types Explained (Malwarebytes)", url="#placeholder_cyber_m6_s1_mb", resource_type="Resource", step_id=s6_1_cy.id))
                    resources_to_add_cyber.append(Resource(name="Social Engineering Attacks (Article)", url="#placeholder_cyber_m6_s2_soceng", resource_type="Article", step_id=s6_2_cy.id))
                    resources_to_add_cyber.append(Resource(name="OWASP Top 10 Project", url="#placeholder_cyber_m6_s3_owasp", resource_type="Resource", step_id=s6_3_cy.id))
                    resources_to_add_cyber.append(Resource(name="Intro to IT Risk Management (Video)", url="#placeholder_cyber_m6_s4_yt", resource_type="Video", step_id=s6_4_cy.id))

                    # M7: Ethical Hacking Concepts
                    ...
                    resources_to_add_cyber.append(Resource(name="Pentest Phases Explained (Article)", url="#placeholder_cyber_m7_s1_pentest", resource_type="Article", step_id=s7_1_cy.id))
                    resources_to_add_cyber.append(Resource(name="Google Dorking / OSINT Basics", url="#placeholder_cyber_m7_s2_osint", resource_type="Guide", step_id=s7_2_cy.id))
                    resources_to_add_cyber.append(Resource(name="Metasploit Unleashed (OffSec)", url="#placeholder_cyber_m7_s3_msf", resource_type="Resource", step_id=s7_3_cy.id))
                    resources_to_add_cyber.append(Resource(name="Ethical Hacking Ethics (Article)", url="#placeholder_cyber_m7_s4_ethics", resource_type="Article", step_id=s7_4_cy.id))

                    # M8: Security Operations & Incident Response Basics
                    ...
                    resources_to_add_cyber.append(Resource(name="Common Log Sources (Article)", url="#placeholder_cyber_m8_s1_logs", resource_type="Article", step_id=s8_1_cy.id))
                    resources_to_add_cyber.append(Resource(name="Log Analysis Tutorial (Video)", url="#placeholder_cyber_m8_s2_yt", resource_type="Video", step_id=s8_2_cy.id))
                    resources_to_add_cyber.append(Resource(name="Incident Response Steps (SANS)", url="#placeholder_cyber_m8_s3_sans", resource_type="Guide", step_id=s8_3_cy.id))
                    resources_to_add_cyber.append(Resource(name="Day in the Life of a SOC Analyst (YT)", url="#placeholder_cyber_m8_s4_yt", resource_type="Video", step_id=s8_4_cy.id))

                    # M9: Compliance & Frameworks Awareness
                    ...
                    resources_to_add_cyber.append(Resource(name="NIST CSF Overview (NIST)", url="#placeholder_cyber_m9_s1_nist", resource_type="Resource", step_id=s9_1_cy.id))
                    resources_to_add_cyber.append(Resource(name="ISO 27001 Explained (Article)", url="#placeholder_cyber_m9_s2_iso", resource_type="Article", step_id=s9_2_cy.id))
                    resources_to_add_cyber.append(Resource(name="NDPR Overview (NITDA - Placeholder)", url="#placeholder_cyber_m9_s3_nitda", resource_type="Resource", step_id=s9_3_cy.id))

                    # M10: Career Prep & Certifications (Security+)
                    ...
                    s10_2_cy = Step(name="Study Key Security+ Domains (Using Free Resources)", sequence=20, estimated_time_minutes=2400, milestone_id=m10_cy.id, step_type='Certificate')
                    ...
                    resources_to_add_cyber.append(Resource(name="Security+ SY0-701 Objectives (CompTIA)", url="#placeholder_cyber_m10_s1_secplus", resource_type="Guide", step_id=s10_1_cy.id))
                    resources_to_add_cyber.append(Resource(name="Professor Messer Security+ Training (YT)", url="#placeholder_cyber_m10_s2_messer", resource_type="Course", step_id=s10_2_cy.id))
                    resources_to_add_cyber.append(Resource(name="Cybersecurity Resume Guide (Blog)", url="#placeholder_cyber_m10_s3_resume", resource_type="Article", step_id=s10_3_cy.id))
                    resources_to_add_cyber.append(Resource(name="Security Analyst Interview Qs (Article)", url="#placeholder_cyber_m10_s4_interview", resource_type="Article", step_id=s10_4_cy.id))

                    if resources_to_add_cyber: db.session.add_all(resources_to_add_cyber)
                    db.session.commit()
                    print(f"Path '{cyber_path.name}' seeded successfully.")
                except Exception as e:
                    db.session.rollback(); print(f"Error seeding path '{cyber_path.name}': {e}")
            else: print("Cyber Path not found.")
            # --- <<< END CYBERSECURITY SEEDING BLOCK >>> ---


            # --- Re-Seed Software Engineering Path ---
            print("Attempting to seed Software Engineering path...")
            swe_path = CareerPath.query.filter_by(name="Software Engineering").first()
            if swe_path:
                resources_to_add_swe = []
                try:
                    # M1: Programming Fundamentals (Python)
                    ...
                    resources_to_add_swe.append(Resource(name="Official Python Tutorial", url="https://docs.python.org/3/tutorial/", resource_type="Documentation", step_id=s1_1_swe.id))
                    resources_to_add_swe.append(Resource(name="Python Control Flow (Real Python - Placeholder)", url="#placeholder_swe_m1_s2_rp", resource_type="Article", step_id=s1_2_swe.id))
                    resources_to_add_swe.append(Resource(name="Python Functions Guide (Video - Placeholder)", url="#placeholder_swe_m1_s3_yt", resource_type="Video", step_id=s1_3_swe.id))
                    resources_to_add_swe.append(Resource(name="Python OOP Basics (Article - Placeholder)", url="#placeholder_swe_m1_s4_article", resource_type="Article", step_id=s1_4_swe.id))
                    resources_to_add_swe.append(Resource(name="Python Exceptions Handling (Docs)", url="https://docs.python.org/3/tutorial/errors.html", resource_type="Documentation", step_id=s1_5_swe.id))

                    # M2: Data Structures & Algorithms Basics
                    ...
                    resources_to_add_swe.append(Resource(name="Python Data Structures In-Depth (Article - Placeholder)", url="#placeholder_swe_m2_s1_article", resource_type="Article", step_id=s2_1_swe.id))
                    resources_to_add_swe.append(Resource(name="Big O Notation Explained (Video - Placeholder)", url="#placeholder_swe_m2_s2_yt", resource_type="Video", step_id=s2_2_swe.id))
                    resources_to_add_swe.append(Resource(name="Searching Algorithms (GeeksForGeeks - Placeholder)", url="#placeholder_swe_m2_s3_gfg", resource_type="Article", step_id=s2_3_swe.id))
                    resources_to_add_swe.append(Resource(name="Sorting Algorithms Visualized", url="#placeholder_swe_m2_s4_sortviz", resource_type="Resource", step_id=s2_4_swe.id))
                    resources_to_add_swe.append(Resource(name="LeetCode Easy Problems", url="https://leetcode.com/problemset/all/?difficulty=EASY&page=1", resource_type="Platform", step_id=s2_5_swe.id))

                    # M3: Version Control (Git)
                    ...
                    resources_to_add_swe.append(Resource(name="Pro Git Book (Free Online)", url="https://git-scm.com/book/en/v2", resource_type="Book", step_id=s3_1_swe.id))
                    ...
                    resources_to_add_swe.append(Resource(name="GitHub Docs - Working with Remotes", url="#placeholder_swe_m3_s4_github", resource_type="Documentation", step_id=s3_4_swe.id))
                    ...
                    resources_to_add_swe.append(Resource(name="How the Internet Works (Video - Placeholder)", url="#placeholder_swe_m4_s1_yt", resource_type="Video", step_id=s4_1_swe.id))
                    ...
                    # M5: Backend Development Framework (Flask)
                    ...
                    resources_to_add_swe.append(Resource(name="Flask Quickstart (Official Docs)", url="https://flask.palletsprojects.com/en/3.0.x/quickstart/", resource_type="Documentation", step_id=s5_1_swe.id))
                    ...
                    resources_to_add_swe.append(Resource(name="What is REST? (Article - Placeholder)", url="#placeholder_swe_m7_s1_rest", resource_type="Article", step_id=s7_1_swe.id))
                    resources_to_add_swe.append(Resource(name="Build Flask API (Tutorial - Placeholder)", url="#placeholder_swe_m7_s2_flaskapi", resource_type="Tutorial", step_id=s7_2_swe.id))
                    resources_to_add_swe.append(Resource(name="API Authentication Explained (Blog)", url="#placeholder_swe_m7_s3_apiauth", resource_type="Article", step_id=s7_3_swe.id))
                    ...
                    resources_to_add_swe.append(Resource(name="Why Write Tests? (Blog - Placeholder)", url="#placeholder_swe_m8_s1_whytest", resource_type="Article", step_id=s8_1_swe.id))
                    ...
                    resources_to_add_swe.append(Resource(name="Python Mocking Intro (Real Python - Placeholder)", url="#placeholder_swe_m8_s4_rp", resource_type="Article", step_id=s8_4_swe.id))

                    # M9: Deployment Concepts
                    ...
                    resources_to_add_swe.append(Resource(name="PaaS vs IaaS (Article - Placeholder)", url="#placeholder_swe_m9_s4_paas", resource_type="Article", step_id=s9_4_swe.id))
                    resources_to_add_swe.append(Resource(name="Using Environment Variables (Blog - Placeholder)", url="#placeholder_swe_m9_s5_envvars", resource_type="Article", step_id=s9_5_swe.id))

                    # M10: Building & Showcasing Projects
                    ...
                    s10_2_swe = Step(name="Build Project using Flask, SQLAlchemy, etc.", sequence=20, estimated_time_minutes=1800, milestone_id=m10_swe.id, step_type='Project')
                    ...
                    resources_to_add_swe.append(Resource(name="Flask Project Ideas (Blog - Placeholder)", url="#placeholder_swe_m10_s1_ideas", resource_type="Article", step_id=s10_1_swe.id))
                    resources_to_add_swe.append(Resource(name="Example Flask Projects (GitHub Search - Placeholder)", url="#placeholder_swe_m10_s2_github", resource_type="Resource", step_id=s10_2_swe.id))
                    resources_to_add_swe.append(Resource(name="Writing Good READMEs (Article - Placeholder)", url="#placeholder_swe_m10_s4_readme", resource_type="Article", step_id=s10_4_swe.id))
                    resources_to_add_swe.append(Resource(name="Deploying Flask to Railway (Docs - Placeholder)", url="#placeholder_swe_m10_s5_railway", resource_type="Documentation", step_id=s10_5_swe.id))

                    # --- Add all collected resources for SWE ---
                    if resources_to_add_swe: db.session.add_all(resources_to_add_swe)
                    db.session.commit()
                    print(f"Path '{swe_path.name}' seeded successfully.")

                except Exception as e:
                    db.session.rollback(); print(f"Error seeding path '{swe_path.name}': {e}")

            else: print("SWE Path not found.")
            # --- <<< END SOFTWARE ENGINEERING SEEDING BLOCK >>> ---

            flash("Database has been reset and re-seeded.", 'warning')
            return redirect(url_for('home')) # Redirect home after completion

    except Exception as e:
        db.session.rollback() # Rollback outer transaction if drop/create fails
        print(f"Error during FULL DB reset: {e}") # Log the specific error
        flash(f"Error during database reset: {e}", 'danger')
        return redirect(url_for('home')) # Redirect home even on error

# --- <<< END TEMPORARY ADMIN RESET ROUTE >>> ---
