...
# IMPORTANT: Set this key securely in your environment variables or replace the placeholder
...
def init_database():
    ...
    # Example: if key_from_url != INIT_DB_SECRET_KEY: abort(404)

    print("Attempting database initialization and seeding...")
    try:
        with app.app_context():
            # Create all tables that don't exist (including portfolio_items now)
            db.create_all()
            ...
            # --- Seed Data Analytics Path (Only if Milestones don't exist for it yet) ---
            ...
            if da_path and not Milestone.query.filter_by(career_path_id=da_path.id).first():
                ...
                resources_to_add.append(Resource(name="Excel Guided Project (Coursera - Placeholder)", url="#placeholder_coursera_link_5", resource_type="Project", step_id=s2_5.id))
                ...
                db.session.add_all([s7_1, s7_2, s7_3, s7_4]); db.session.flush()
                 # !!! REPLACE PLACEHOLDER URLs BELOW !!!
                resources_to_add.append(Resource(name="How I Structure My Data Analytics Reports (Medium - Placeholder)", url="#placeholder_medium_article_structuring", resource_type="Article", step_id=s7_1.id))
                ...
                resources_to_add.append(Resource(name="Free Resume Template (Placeholder)", url="#placeholder_bitly_link_resume", resource_type="Resource", step_id=s9_1.id))
